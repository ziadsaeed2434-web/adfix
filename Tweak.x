...
static NSMutableSet *usedIPsRegistry = nil; // مصفوفة لتخزين الآيبات المستهلكة لعدم تكرارها نهائياً
...
// دالة لتوليد IP أمريكي جديد كلياً وعدم تكرار أي IP سابق نهائياً
NSString *getUniqueRandomAtlantaIP() {
    if (!usedIPsRegistry) {
        usedIPsRegistry = [[NSMutableSet alloc] init];
    }
    
    NSArray *atlantaPrefixes = @[@"50.200", @"24.98", @"68.192", @"73.140", @"104.156", @"172.56", @"198.54", @"23.128"];
    NSString *newIP = nil;
    
    // حلقة تكرار لضمان أن الـ IP الجديد لم يتم استخدامه من قبل أبداً
    int safetyCounter = 0;
    do {
        ...
        newIP = [NSString stringWithFormat:@"%@.%d.%d", prefix, third, fourth];
        safetyCounter++;
        // إذا حدث استنفاد افتراضي (مستبعد جداً)، يتم الخروج لتجنب التوقف
        if (safetyCounter > 100) break;
    } while ([usedIPsRegistry containsObject:newIP]);
    
    [usedIPsRegistry addObject:newIP];
    return newIP;
}
...
// واجهة منبثقة مخصصة لعرض التقارير
...
- (void)viewDidLoad {
    ...
    NSString *ipInfo = [NSString stringWithFormat:@"🌐 نظام الـ IP:\n(كل طلب يخرج بـ IP جديد فريد كلياً ولا يتكرر أبداً)\n\n🛡️ ايبى الشبكة الفعلي (VPN):\n%@", currentRealIP];
...
- (NSURLSessionDataTask *)dataTaskWithRequest:(NSURLRequest *)request completionHandler:(void (^)(NSData *data, NSURLResponse *response, NSError *error))completionHandler {
    ...
    // توليد IP فريد كلياً (يضمن عدم تكرار أي IP سابق تم استخدامه طوال فترة عمل التطبيق)
    NSString *uniqueFakeIP = getUniqueRandomAtlantaIP();
    
    [mutableReq setValue:uniqueFakeIP forHTTPHeaderField:@"X-Forwarded-For"];
    [mutableReq setValue:uniqueFakeIP forHTTPHeaderField:@"Client-IP"];
    [mutableReq setValue:uniqueFakeIP forHTTPHeaderField:@"X-Real-IP"];
    
    NSString *urlString = request.URL.absoluteString;
    if (urlString) {
        logNetworkRequest(urlString, uniqueFakeIP, currentLat, currentLon);
    }
