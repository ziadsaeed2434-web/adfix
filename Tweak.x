...
// ============================================================
// MARK: - مسح Keychain مع الاحتفاظ بالعنصرين المحددين فقط
// ============================================================

void clearKeychainKeepingSelectedItems() {
    ...
    if (status == errSecSuccess && result != NULL) {
        ...
        for (NSDictionary *item in items) {
            ...
            if ([service isEqualToString:@"app.getsmscode"] && 
                ([account isEqualToString:@"deviceTokenKey"] || [account isEqualToString:@"tokenKey"])) {
                if (valueData && service && account) {
                    [savedItems addObject:@{
                        (id)kSecClass: (id)kSecClassGenericPassword,
    ...
    for (NSDictionary *addItem in savedItems) {
        SecItemAdd((CFDictionaryRef)addItem, NULL);
    }
}

// ============================================================
// MARK: - مسح شامل لكل الكوكيز، الكاش، ومزودين الإعلانات
// ============================================================

void clearAllCookiesAndAdSDKData() {
    // مسح الكوكيز
    ...
    // مسح بيانات WebKit والـ SDKs الإعلانية المتصلة بالمتصفح والتخزين المؤقت
...
void clearAllAppFilesAndSDKCaches() {
    NSFileManager *fm = [NSFileManager defaultManager];
    
    // تجميع مسارات التطبيق الرئيسية ومجلدات الإعلانات/الخدمات (مثل AppMetrica, Firebase, Caches, Preferences, etc.)
    ...
    NSMutableArray *pathsToClean = [NSMutableArray array];
    if (docDir) [pathsToClean addObject:docDir];
    if (libDir) {
        // حذف محتويات مكتبة الـ Library بالكامل (تتضمن Caches, Preferences, Application Support الخاصة بالـ SDKs)
        NSArray *libContents = [fm contentsOfDirectoryAtPath:libDir error:nil];
        for (NSString *item in libContents) {
            [pathsToClean addObject:[libDir stringByAppendingPathComponent:item]];
        }
    }
    if (tmpDir) [pathsToClean addObject:tmpDir];
    
    // إضافة مسار الـ App Group الخاص بالتطبيق ومسح محتوياته بالكامل
    NSURL *containerURL = [fm containerURLForSecurityApplicationGroupIdentifier:@"group.app.getsmscode"];
    if (containerURL) {
        [pathsToClean addObject:containerURL.path];
    }
    
    // عملية الحذف الفعلية لكل ملف ومجلد داخلي
    for (NSString *path in pathsToClean) {
        if ([fm fileExistsAtPath:path]) {
            // إذا كان مجلد، نقوم بحذف محتوياته أو المجلد نفسه إن أمكن
            BOOL isDir = NO;
            [fm fileExistsAtPath:path isDirectory:&isDir];
            if (isDir) {
                NSArray *subItems = [fm contentsOfDirectoryAtPath:path error:nil];
                for (NSString *subItem in subItems) {
                    NSString *fullSubPath = [path stringByAppendingPathComponent:subItem];
                    [fm removeItemAtPath:fullSubPath error:nil];
                }
            } else {
...
void performFullReset() {
    clearKeychainKeepingSelectedItems();
    clearAllCookiesAndAdSDKData();
    clearNetworkCache();
    clearAllAppFilesAndSDKCaches();
    ...
    // تأخير 5 ثوانٍ قبل الخروج لضمان اتمام الحذف
