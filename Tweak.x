...
// 🌐 توليد IP حقيقي وصحيح ديناميكياً ضمن نطاقي 172.57 أو 172.59 للجلسة الحالية
void initializeSessionIP() {
    ...
    sessionFakeIP = [NSString stringWithFormat:@"172.%d.%d.%d", secondOctet, thirdOctet, fourthOctet];
}
...
- (void)viewDidLoad {
    ...
    NSString *ipInfo = [NSString stringWithFormat:@"🌐 IP الجلسة الوهمي الحالي (مولد ديناميكياً):\n%@\n\n🛡️ ايبى الشبكة الفعلي (VPN):\n%@", sessionFakeIP, currentRealIP];
...
%ctor {
    ...
    initializeSessionIP(); // توليد آبي حقيقي وصحيح للجلسة طوال فترة فتح التطبيق
    fetchRealIP();
