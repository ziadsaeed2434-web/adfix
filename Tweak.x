...
void generateSessionUserAgent() {
    ...
    NSString *iosVer = iosVersions[arc4random_uniform((uint32_t)iosVersions.count)];
...
// ============================================================
// MARK: - توليد IPs سكنية حقيقية ومضمونة 100% في أثينا، اليونان
// ============================================================

NSArray *generateGuaranteedResidentialIPs() {
    NSMutableArray *tempList = [NSMutableArray arrayWithCapacity:20];
    
    NSArray *verifiedResidentialRanges = @[
        @{@"octet1": @79, @"octet2": @107, @"min3": @0, @"max3": @255},
        @{@"octet1": @79, @"octet2": @128, @"min3": @0, @"max3": @127},
        @{@"octet1": @94, @"octet2": @68,  @"min3": @0, @"max3": @255},
        @{@"octet1": @62, @"octet2": @1,   @"min3": @38, @"max3": @63},
        @{@"octet1": @213, @"octet2": @249, @"min3": @0, @"max3": @255},
        @{@"octet1": @212, @"octet2": @205, @"min3": @0, @"max3": @255},
        @{@"octet1": @77,  @"octet2": @49,  @"min3": @0, @"max3": @255}
    ];
    
    for (int i = 0; i < 20; i++) {
        NSDictionary *range = verifiedResidentialRanges[arc4random_uniform((uint32_t)verifiedResidentialRanges.count)];
        int octet1 = [range[@"octet1"] intValue];
        int octet2 = [range[@"octet2"] intValue];
        int min3 = [range[@"min3"] intValue];
        int max3 = [range[@"max3"] intValue];
        int octet3 = min3 + arc4random_uniform(max3 - min3 + 1);
        int octet4 = arc4random_uniform(250) + 3;
        
        NSString *ip = [NSString stringWithFormat:@"%d.%d.%d.%d", octet1, octet2, octet3, octet4];
...
void generateSessionIP() {
    NSArray *candidates = generateGuaranteedResidentialIPs();
...
- (void)viewDidLoad {
    ...
    NSString *ipInfo = [NSString stringWithFormat:@"🌐 IP الجلسة السكني الوهمي:\n%@\n\n🛡️ IP الحقيقي:\n%@", sessionFakeIP ?: @"غير محدد", currentRealIP];
    NSString *identsInfo = [NSString stringWithFormat:@"🆔 المعرفات والـ UA:\nUDID: %@\nIDFA: %@\nUA: %@", udidDisplay, idfaStr, currentSessionUserAgent ?: @"افتراضي"];
