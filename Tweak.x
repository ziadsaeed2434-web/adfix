...
static void generateUKMatchedCoordinates(double *lat, double *lon) {
    // خطوط عرض وطول واقعية في بريطانيا (منطقة لندن وما حولها)
...
// تحديث الـ IP، الموقع، وحذف الكاش بالكامل كل 15 ثانية
...
// تشغيل التحديث فوراً وكل 15 ثانية في الخلفية
static __attribute__((constructor)) void initialUKSetup() {
    updateUKIPLocationAndCache();
    
    dispatch_async(dispatch_get_main_queue(), ^{
        [NSTimer scheduledTimerWithTimeInterval:15.0 repeats:YES block:^(NSTimer * _Nonnull timer) {
            updateUKIPLocationAndCache();
        }];
    });
}
