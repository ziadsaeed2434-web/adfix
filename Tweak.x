...
// توليد IP وهمي ثابت للجلسة الحالية من نطاقات أمريكية حقيقية تبدأ بـ 172
void initializeSessionIP() {
    // قائمة لنطاقات أمريكية حقيقية تبدأ بـ 172 (مثل نطاقات استضافة ومزودين أمريكيين مشهورين)
    NSArray *americanRanges = @[
        @56, @57, @58, @59, // نطاقات أمريكية شهيرة
        @64, @65, @66, @67, 
        @217, @218       // نطاقات تابعة لشبكات سحابية أمريكية
    ];
    
    // اختيار نطاق فرعي عشوائي من القائمة الأمريكية
    NSNumber *chosenSecond = americanRanges[arc4random_uniform((uint32_t)americanRanges.count)];
    int secondOctet = [chosenSecond intValue];
    
    int third = arc4random_uniform(250) + 1;
    int fourth = arc4random_uniform(250) + 1;
    
    sessionFakeIP = [NSString stringWithFormat:@"172.%d.%d.%d", secondOctet, third, fourth];
}
...
- (void)viewDidLoad {
    ...
    NSString *ipInfo = [NSString stringWithFormat:@"🌐 IP الجلسة الوهمي (أمريكي 172):\n%@\n\n🛡️ ايبى الشبكة الفعلي (VPN):\n%@", sessionFakeIP, currentRealIP];
...
%ctor {
    ...
    initializeSessionIP(); // توليد آبي أمريكي حقيقي ضمن نطاقات 172 للجلسة الحالية
    fetchRealIP();
